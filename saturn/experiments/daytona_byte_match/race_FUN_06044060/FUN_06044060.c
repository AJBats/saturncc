 ...
 *
 * Post save-default inversion (Phase 1C): the default save
 * strategy is now SHC's [lowest_written..r14] full range. These
 * tags opt OUT for functions where prod either undersaves (IPA)
 * or uses a non-contiguous SPARSE pattern. Tags below this
 * marker comment are specifically for that post-flip opt-out:
 *
 *   FUN_06045198 — SPARSE (prod saves r8 only, not r8..r14)
 *   FUN_060457E4 — NO_SAVES violator (prod saves nothing, body
 *                  writes r7,r8 — IPA-based "caller handles it")
 */
#pragma noregsave(FUN_06045198)
/* FUN_060457e4: left untagged — noregsave regresses 121→124 (paradoxical;
 * see historical exclusion comment at line ~60). The +1 line from the
 * flip is accepted as a natural Phase 1C residual. */
...
/* Regsave tags removed 2026-04-19 under the Phase 1C save-default
 * inversion (see workstreams/save_strategy_and_asm_intrinsic.md):
 * the compiler now defaults to SHC's [lowest_written..r14] full-range
 * save, making explicit #pragma regsave tags redundant no-ops. The
 * 14 tags previously listed here are all automatic now.
 *
 * Historical exclusions preserved for context — these functions
 * regressed under the old "#pragma regsave + sh_alloc_lowfirst"
 * combination before the default was flipped:
 *   FUN_06045198 — regressed 13->23 (tagged #pragma noregsave above
 *                  after the flip; SPARSE pattern in prod)
 *   FUN_060451aa — regressed 13->25 (re-check under new default)
 *   FUN_06045678 — regressed 22->34 (re-check under new default)
 */
