/* sh.c - Hitachi-style SH-2 backend for LCC v4.2 (SaturnCompiler project).
 *
 * Phase 1A: stub backend. Based on null.c skeleton. All functions are
 * no-ops — this target registers successfully and runs without crashing,
 * but emits no code. It's here to validate the plumbing (Makefile, bind.c
 * registration, Interface wiring) before Phase 1B replaces it with a
 * real lburg grammar.
 *
 * Next: convert to src/sh.md with lburg rules for SH-2 codegen,
 * targeting Hitachi SHC v5.0-compatible output.
 *
 * See workstreams/sh_backend_sketch.md and workstreams/lcc_feasibility.md.
 */
#include "c.h"
#define I(f) sh_##f
